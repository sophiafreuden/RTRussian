...
data <- read_delim("klinton-maidan.txt", delim = "*", col_types = col_types)
...
sums2 %>%
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(сорос)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'сорос' (soros), RT Search Term: 'клинтон майдан' (clinton maidan)",
    x = "Date",
    y = "Frequency"
...
multsum2 %>%
  ...
  tab_header(
    ...
    subtitle = "RT Search Term: 'клинтон майдан' (clinton maidan)"
    )  %>% 
  tab_source_note(
