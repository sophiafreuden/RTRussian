...
sums2 %>%
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(браудера)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'браудера' (browder), RT Search Term: 'клинтон протесты' (clinton protests)",
    x = "Date",
    y = "Frequency",
  ...
  ggsave("klinton-protesty/plot1.png", width = 10)
```

```{r}
# Change min.freq below as needed per resultsno pulled from Python scrape.
wordcloud(words = head$word, freq = head$count, min.freq = 18,
