...
```{r}
choice2 <- sums2 %>% select(c(ArtDate, западного, URL))
...
sums2 %>%
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(западного)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  # geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'западного' (western), RT Search Term: N/A, 'Клинтон' (Clinton) Terms",
    x = "Date",
    y = "Frequency"
  ...
  ggsave("all-klinton/western2.png", width = 10)
```
