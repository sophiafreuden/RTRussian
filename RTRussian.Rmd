...
data <- read_delim("klinton-grazhdanskoe-obschestvo.txt", delim = "*", col_types = col_types)
...
sums2 %>%
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(сша)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'сша' (usa)
RT Search Term: 'клинтон гражданское общество' (clinton civil society)",
    x = "Date",
    y = "Frequency"
  ...
  ggsave("klinton-grazhdanskoe-obschestvo/usa.png", width = 10)
```
