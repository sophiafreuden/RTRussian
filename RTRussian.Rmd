...
data <- read_delim("soros.txt", delim = "*", col_types = col_types)
...
sums2 %>%
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(демократическойпартии)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'демократическойпартии' (democraticparty), RT Search Term: 'сорос' (soros)",
    x = "Date",
    y = "Frequency",
...
multsum2 %>%
  ...
  tab_header(
    ...
    subtitle = "RT Search Term: 'сорос' (soros)"
    )  %>% 
  tab_source_note(
