...
data <- read_delim("makfol.txt", delim = "*", col_types = col_types)
...
sums2 %>%
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(выборов)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'выборов' (elections), RT Search Term: 'макфол' (mcfaul)",
    x = "Date",
    y = "Frequency"
  ...
  ggsave("makfol/plot1.png", width = 10)
```

```{r}
# Change min.freq below as needed per resultsno pulled from Python scrape.
wordcloud(words = head$word, freq = head$count, min.freq = 80,
...
multsum2 %>%
  ...
  tab_header(
    ...
    subtitle = "RT Search Term: 'макфол' (mcfaul)"
    )  %>% 
  tab_source_note(
